        ...
        <%
            ...
            if(cookies!=null){
                for(Cookie c:cookies){ //遍历cookies数组
                    if(c.getName().equals("id")) //判断当前遍历Cookie的名称是否是username
                        uname=c.getValue(); //是的话获取其值
                  if(c.getName().equals("pwd")) //判断当前Cookie的名称是否是password
                        upwd=c.getValue(); //是的话获取其值
                }  }   %>
           ...
           <div class="page">
			...
			<form id="adminLogin" name="adminLogin" method="post" onsubmit="" action="login/adminLogin">
				<li class="login-item">
					<span>登陆 账号：</span>
					<input type="text" id="id" name="id" value="<%=uname==null?"":uname %>"   class="login_input" >
                                        <span id="count-msg" class="error"></span>
				</li>
				<li class="login-item">
					<span>登陆 密码：</span>
					<input type="password" id="pwd" name="pwd"  value="<%=upwd==null?"":upwd %>"  class="login_input" >
